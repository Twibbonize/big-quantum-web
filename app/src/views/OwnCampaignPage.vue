...
const campaignAnalytic = ref(null);
...
const campaignAnalyticSize = useElementBounding(campaignAnalytic);
...
const campaignAnalyticStyle = computed(() => {
    if (!campaignContent.value) {
        return {
            transform: `scale(1) translateY(0)`
        };
    ...
    const targetScale = (windowHeight.value - additionalSpace) / totalContentHeight;
    // const targetHeight = (analyticHeight.value - 20) * targetScale;
    // console.log(targetHeight)
    const translateY = analyticHeight.value * targetScale + 24 + 24;
...
const campaignPageStyle = computedAsync(async () => {
    ...
    const targetHeight = height.value * parsedScale;
    return {
...
useMutationObserver(campaignFeeds, (mutations) => {
    const height = parseFloat(mutations[0].target.style.height);

    const additionalSpace = 88 + 12 + 24; // topY + padding-top + padding-bottom

    const targetScale = (windowHeight.value - additionalSpace) / height;
    const targetHeight = height * targetScale;
    const paddingY = 36 * targetScale

    campaignPage.value.style.height = `${targetHeight + paddingY}px`;
}, {
    attributes: true,
})

// useResizeObserver(campaignContent, (entries) => {
//     const entry = entries[0];
//     const { height } = entry.contentRect;
...
<template>
    <LayoutMain>
        <div class="page campaign">
            ...
            <div ref="campaignPage" class="campaign__content-wrapper">
                <div ref="campaignContent" class="campaign__content" :style="campaignContentStyle">
                            ...
                            <div ref="campaignMain" class="campaign__main">
                                <div class="campaign__frames">
                                    <div class="campaign__frames__stage">
                                        <img :src="selectedFrames" class="campaign__frames__stage__image" />
                                    </div>

                                    <div class="campaign__frames__card">
                                        <RadioGroup v-model="selectedFrames">
                                            <div class="campaign__frames__options">
                                                <RadioGroupOption v-for="(frame, i) in frames" :key="i" :value="frame"
                                                    v-slot="{ checked }">
                                                    <div :class="[
                    ...
                    'campaign__frames__option--checked'
                                                    ...
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20"
                                                        ...
                                                        <g clip-path="url(#clip0_2061_6507)">
                                                            <path
                                                                ...
                                                                stroke="#1B1B1B" stroke-width="1.66667"
                                ...
                                <div class="campaign__detail">
                                    ...
                                    <div class="campaign__detail__creator">
                                        <QCreator name="Hanoi Art 2025" username="hanoiart"
                                            avatar="/assets/img/sample/sampel-avatar-18.jpg" size="md" />
                                    </div>

                                    <p class="campaign__detail__description">
                                        <QEllipsisText text="Welcome to Hanoi Art Book Fair 2023. Our primary mission is to cultivate reading habits,
                                    ...
                                    <div class="campaign__detail__meta">
                                        <div class="campaign__detail__meta-wrapper">
                                            <CampaignMeta title="Supporters" value="95.5K">
                                                <template #icon>
                                                    <svg xmlns="http://www.w3.org/2000/svg" :width="sm ? 20 : 16"
                                                        :height="sm ? 20 : 16" viewBox="0 0 16 16" fill="none">
                                                        <path
                                        ...
                                        <div class="campaign__detail__meta-wrapper">
                                            <CampaignMeta :iconClass="['ri', 'ri-time-line', sm && 'ri-lg'].join(' ')
                    " title="Created" value="5 days ago" />
                                        </div>
                                    </div>
                                    <div class="campaign__detail__actions">
                                        <div class="flex-grow">
                                            <QShareButton link="twibbo.nz/hanoi-art-2025" @click="onClickShare" />
                                        </div>

                                        <div class="flex-shrink-0">
                                            <QButton circle variant="secondary" @click="onClickCollection">
                        ...
                        <div class="col-span-12 md:col-span-7 lg:col-span-8 xl:col-span-9 bg-white md:bg-transparent">
                            <div ref="campaignFeeds" class="campaign-feeds">
                                <div ref="campaignFeedsPanels" class="campaign-feeds__panels">
                                    <div ref="campaignFeedsWrapper" :class="[
                    ...
                    displayType === 'grid'
                        ...
                        : 'campaign-feeds__list'
                                        ...
                                        <PostMockup v-if="!posts.length" v-for="(mock, i) in mocks" :key="i"
                                            v-bind="mock" :display="displayType" :rounded="!sm" />
                                        <PostWrapper v-else v-for="post in posts" :key="post.uri" v-bind="post"
                                            :campaignOwnerPriviledge="false" :display="displayType" :rounded="!sm" />

                                        <QSkeleton v-if="isLoadingPost && displayType === 'grid'" height="100%"
                                            square />
                                        <QSkeleton v-if="isLoadingPost && displayType === 'grid'" height="100%"
                                            square />
                                        <QSkeleton v-if="isLoadingPost && displayType === 'grid'" height="100%"
                                        ...
                                        <QSkeleton v-if="isLoadingPost && displayType === 'list'" height="200px"
                                            rounded />
                                    </div>
                                </div>

                                <div class="campaign-feeds__control">
                                    <QButton circle :variant="sm ? 'secondary' : 'neutral'" @click="toggleDisplay">
                                        <i :class="[
                    displayType === 'grid'
                        ? 'ri-list-unordered'
                        : 'ri-layout-grid-line',
                    ...
                    'font-normal'
                                    ...
                                    <QButton circle v-if="!sm" :variant="sm ? 'secondary' : 'neutral'"
            ...
            <div ref="campaignAnalytic" class="campaign__analytic" :style="campaignAnalyticStyle">
                <div class="campaign__analytic-wrapper">
                    <div class="campaign__analytic-stats">
                        <div class="campaign__analytic-stat">
                            <span class="text-xl font-semibold">2</span>
                            <span class="font-medium mt-1">Supporters</span>
                        </div>
                        <div class="campaign__analytic-stat">
                            <span class="text-xl font-semibold">6</span>
                            <span class="font-medium mt-1">Interactions</span>
                        </div>
                        <div class="campaign__analytic-stat">
                            <span class="text-xl font-semibold">12</span>
                            <span class="font-medium mt-1">Visits</span>
                        </div>
                    </div>
        ...
        <div class="campaign-recommendations bg-gray-50 relative z-10">
            ...
            <div class="container px-4 2xl:px-0 pb-10">
                ...
                <div class="campaign-recommendations__grid">
                    <CampaignCard v-for="campaign in publicCampaigns" :key="campaign.uri" v-bind="campaign" />
                </div>
            </div>
        </div>

        <div class="campaign-edit">
            <div class="flex justify-end mx-auto container px-5 2xl:px-0">
                <QButton variant="secondary">
                    <i class="ri-pencil-line"></i>
                    <span class="ml-2">Edit Campaign</span>
                </QButton>
            </div>
        </div>
    </LayoutMain>
